...
local function teal_source()
   ...
   end; local assert = _tl_compat and _tl_compat.assert or assert; local debug = _tl_compat and _tl_compat.debug or debug; local io =
   _tl_compat and _tl_compat.io or io; local ipairs = _tl_compat and _tl_compat.ipairs or ipairs; local load = _tl_compat and
   _tl_compat.load or load; local math = _tl_compat and _tl_compat.math or math; local _tl_math_maxinteger = math
   .maxinteger or math.pow(2, 53); local os = _tl_compat and _tl_compat.os or os; local package = _tl_compat and
   _tl_compat.package or package; local pairs = _tl_compat and _tl_compat.pairs or pairs; local string = _tl_compat and
   _tl_compat.string or string; local table = _tl_compat and _tl_compat.table or table; local type = type; local utf8 =
   _tl_compat and _tl_compat.utf8 or utf8
...
do
   ...
   do
      ...
      local function fail(ps, i, msg)
         ...
         table.insert(ps.errs,
            { filename = ps.filename, y = ps.tokens[i].y, x = ps.tokens[i].x, msg = assert(msg,
               "syntax error, but no error message provided") })
      ...
      local function verify_end(ps, i, istart, node)
         if ps.tokens[i].tk == "end" then
            ...
            if node.kind ~= "function" and endy ~= node.y and endx ~= node.x then
               if not ps.end_alignment_hint then
                  ps.end_alignment_hint = { filename = ps.filename, y = node.y, x = node.x, msg =
                  "syntax error hint: construct starting here is not aligned with its 'end' at " ..
                  ps.filename .. ":" .. endy .. ":" .. endx .. ":" }
               end
            end
            return i + 1
         end
         ...
         return fail(ps, i,
            ...
            ps.filename .. ":" .. ps.tokens[istart].y .. ":" .. ps.tokens[istart].x .. ":")
      end
            ...
            elseif not close[ps.tokens[i].tk] then
               ...
               if first ~= "}" and ps.tokens[i].y ~= ps.tokens[i - 1].y then
                  table.insert(ps.tokens, i,
                     { tk = first, y = ps.tokens[i - 1].y, x = ps.tokens[i - 1].x + 1, kind = "keyword" })
                  return i, list
               end
            end
         end
         ...
         local function P(ps, i)
            ...
            while true do
               ...
               elseif tkop.tk == "as" or tkop.tk == "is" then
                  ...
                  e1 = { f = ps.filename, y = tkop.y, x = tkop.x, kind = "op", op = op, e1 = e1, e2 = cast, conststr = e1
                  .conststr }
               else
                  break
               end
      ...
      local function parse_function(ps, i, fk)
         ...
         if #names > 1 then
            ...
            for i2 = 2, #names - 1 do
               ...
               owner = { f = ps.filename, y = names[i2].y, x = names[i2].x, kind = "op", op = dot, e1 = owner, e2 = names
               [i2] }
            end
            fn.fn_owner = owner
         end
         ...
         if fn.is_method and fn.args then
            table.insert(fn.args, 1,
               { f = ps.filename, x = selfx, y = selfy, tk = "self", kind = "identifier", is_self = true })
            ...
            elseif parse_type_body_fns[tn] and ps.tokens[i + 1].tk ~= ":" then
               ...
               elseif ps.tokens[i].tk == "=" then
                  ...
                  elseif next_word == "functiontype" then
                     return fail(ps, i,
                        "syntax error: this syntax is no longer valid; use 'type " .. v.tk .. " = function('...")
                  else
         ...
         if ps.tokens[i].tk == "=" then
            ...
            if parse_type_body_fns[tn] then
               ...
               return failskip(ps, i + 1,
                  ...
                  scope .. " " .. next_word .. " " .. asgn.vars[1].tk .. "'", skip_type_body)
            elseif next_word == "functiontype" then
               local scope = node_name == "local_declaration" and "local" or "global"
               return failskip(ps, i + 1,
                  "syntax error: this syntax is no longer valid; use '" ..
                  scope .. " type " .. asgn.vars[1].tk .. " = function('...", parse_function_type)
            end
      ...
      local function type_needs_local_or_global(ps, i)
         local tk = ps.tokens[i].tk
         return failskip(ps, i, ("%s needs to be declared with 'local %s' or 'global %s'"):format(tk, tk, tk),
            skip_type_body)
      end
   ...
   function Errors:redeclaration_warning(at, var_name, var_kind, old_var)
      ...
      if old_var and old_var.declared_at then
         self:add_warning("redeclaration", at, short_error .. " (originally declared at %d:%d)", var_name,
            old_var.declared_at.y, old_var.declared_at.x)
      else
      ...
      for name, var in pairs(vars) do
         ...
         if var.declared_at and not has_var_been_used(var) then
            if var.used_as_type then
               ...
               usage_warnings = usage_warnings or {}
               table.insert(usage_warnings,
                  { y = var.declared_at.y, x = var.declared_at.x, name = name, var = var, kind = "unused" })
            end
   ...
   function Errors:check_redeclared_key(w, ctx, seen_keys, key)
      if key ~= nil then
         ...
         if s then
            self:add_in_context(w, ctx,
               "redeclared key " ..
               tostring(key) .. " (previously declared at " .. self.filename .. ":" .. s.y .. ":" .. s.x .. ")")
         else
            seen_keys[key] = w
         end
      end
   ...
   local function inferred_msg(t, prefix)
      return " (" ..
      (prefix or "") .. "inferred at " .. t.inferred_at.f .. ":" .. t.inferred_at.y .. ":" .. t.inferred_at.x .. ")"
   end
   ...
   local function require_module(w, module_name, opts, env)
      ...
      if found and (opts.feat_lax == "on" or found:match("tl$")) then
         ...
         local defaults = {
            ...
            run_internal_compiler_checks = opts.run_internal_compiler_checks or
            save_defaults.run_internal_compiler_checks,
         }
   ...
   local function add_compat_entries(program, used_set, gen_compat)
      ...
      for _, name in ipairs(used_list) do
         ...
         elseif name == "bit32" then
            load_code(name,
               "local bit32 = bit32; if not bit32 then local p, m = " .. req("bit32") .. "; if p then bit32 = m end")
         elseif name == "mt" then
   ...
   do
            ...
            for fname, ftype in pairs(rec.meta_fields) do
               if ret.fields[fname] then
                  if not self:is_a(ftype, ret.fields[fname]) then
                     self.errs:add(ftype, fname .. " does not follow metatable contract: got %s, expected %s", ftype,
                        ret.fields[fname])
                  end
               end
         ...
         function TypeChecker:are_same_nominals(t1, t2)
            ...
            if t1.typevals and t2.typevals and #t1.typevals == #t2.typevals then
               ...
               for i = 1, #t1.typevals do
                  ...
                  self.errs:add_prefixing(nil, typeval_errs, "type parameter <" .. show_type(t2.typevals[i]) .. ">: ",
                     errs)
               end
         ...
         if vt then
            ...
            if constraint then
               if not self:is_a(other, constraint) then
                  return false,
                      { Err(
                      "given type %s does not satisfy %s constraint in type variable " ..
                      display_typevar(typevar, "typevar"), other, constraint) }
               end
            ...
            ["tupletable"] = function(self, a, b)
               for i = 1, math.min(#a.types, #b.types) do
                  if not self:same_type(a.types[i], b.types[i]) then
                     return false,
                         { Err("in tuple entry " .. tostring(i) .. ": got %s, expected %s", a.types[i], b.types[i]) }
                  end
               end
            ...
            ["function"] = function(self, a, b)
               ...
               if naargs ~= nbargs then
                  if (not not a.is_method) ~= (not not b.is_method) then
                     return false,
                         { Err("different number of input arguments: method and non-method are not the same type") }
                  end
                  return false,
                      { Err("different number of input arguments: got " ..
                      naargs - argdelta .. ", expected " .. nbargs - argdelta) }
               end
         ...
         ["tupletable"] = {
            ...
            ["array"] = function(self, a, b)
               if b.inferred_len and b.inferred_len > #a.types then
                  return false,
                      { Err("incompatible length, expected maximum length of " ..
                      tostring(#a.types) .. ", got " .. tostring(b.inferred_len)) }
               end
         ...
         ["array"] = {
            ...
            ["tupletable"] = function(self, a, b)
               ...
               if alen > #b.types then
                  return false,
                      { Err("incompatible length, expected maximum length of " ..
                      tostring(#b.types) .. ", got " .. tostring(alen)) }
               end
               ...
               for i = 1, (alen > 0) and alen or #b.types do
                  if not self:is_a(a.elements, b.types[i]) then
                     return false,
                         { Err("tuple entry " .. i .. " of type %s does not match type of array elements, which is %s",
                            b.types[i], a.elements) }
                  end
               end
               return true
            end,
            ...
            ["function"] = function(self, a, b)
               ...
               if (not b.args.is_va) and (self.feat_arity and (#aa > #ba and a.min_arity > b.min_arity)) then
                  table.insert(errs,
                     ...
                     show_arity(a) .. " %s, expected " .. show_arity(b) .. " %s", a.args, b.args))
               else
               ...
               if #ar < #br and not diff_by_va then
                  table.insert(errs,
                     Err("incompatible number of returns: got " .. #ar .. " %s, expected " .. #br .. " %s", a.rets,
                        b.rets))
               else
                  local nrets = #br
                  if diff_by_va then
      ...
      do
            ...
            for _, a in ipairs(typeargs) do
               if not self:find_var_type(a.typearg) then
                  if a.constraint then
                     ...
                     self:add_var(nil, a.typearg,
                        self.feat_lax and a_type(a, "unknown", {}) or a_type(a, "unresolvable_typearg", {
                           typearg = a.typearg,
                        }))
                  end
               end
         ...
         do
            ...
            do
               ...
               check_args_rets = function(self, w, wargs, f, args, expected_rets, argdelta, or_args, or_rets)
                  ...
                  args_ok, args_errs = check_func_type_list(self, w, wargs, fargs, args, from, argdelta, "contravariant",
                     "argument")
            ...
            local function is_method_mismatch(self, w, arg1, farg1, cm)
               ...
               if cm == "plain" then
                  self.errs:add_warning("hint", w,
                     "invoked method as a regular function: consider using ':' instead of '.'")
               end
               return false
            end
            ...
            check_call = function(self, w, wargs, f, args, expected_rets, cm, argdelta, or_args, or_rets)
               ...
               if given < min_arity or (given > wanted and not (or_args or f.args).is_va) then
                  return nil,
                      { Err_at(w, "wrong number of arguments (given " .. given .. ", expects " .. show_arity(f) .. ")") }
               end
         ...
         do
            local function fail_poly_call_arity(self, w, p, given)
               ...
               return { Err_at(w,
                  "wrong number of arguments (given " .. given .. ", expects " .. table.concat(expects, " or ") .. ")") }
            end
               ...
               for pass = 1, 3 do
                  for i, f in self:iterate_poly(p) do
                     ...
                     if (not tried[i]) and
                        ...
                        local ok, errs = check_call(self, w, wargs, f, args, expected_rets, cm, argdelta, or_args,
                           or_rets)
                        if ok then
      ...
      function TypeChecker:add_global(node, varname, valtype, is_assigning)
         ...
         if existing then
            ...
            elseif valtype and not self:same_type(existing.t, valtype) then
               self.errs:add(node,
                  "cannot redeclare global with a different type: previous type of " .. varname .. " is %s", existing.t)
            end
            return nil
         end
      ...
      function TypeChecker:type_check_index(anode, bnode, a, b)
         ...
         elseif ra.typename == "emptytable" then
            ...
            errm, erra, errb = "inconsistent index type: got %s, expected %s" .. inferred_msg(ra.keys, "type of keys "),
                b, ra.keys
         ...
         not_facts = function(self, fs)
            ...
            for var, f in pairs(fs) do
               ...
               elseif not self:is_a(f.typ, typ) then
                  assert(f.fact == "is")
                  self.errs:add_warning("branch", f.w, f.var .. " (of type %s) can never be a %s", show_type(typ),
                     show_type(f.typ))
                  ret[var] = EqFact({ var = var, typ = a_type(f.w, "invalid", {}), w = f.w, no_infer = f.no_infer })
               else
                  assert(f.fact == "is")
                  ret[var] = IsFact({ var = var, typ = subtract_types(self, f.w, typ, f.typ), w = f.w, no_infer = f
                  .no_infer })
               end
            end
            return ret
      ...
      local special_functions = {
            ...
            if not (t.typename == "map") then
               if not (self.feat_lax and is_unknown(t)) then
                  if t.fields then
                     ...
                     self.errs:add_warning("hint", node.e2,
         ...
         ["rawget"] = function(self, node, _a, b, _argdelta)
            if #b.tuple == 2 then
               return a_type(node, "tuple",
                  { tuple = { self:type_check_index(node.e2[1], node.e2[2], b.tuple[1], b.tuple[2]) } })
            else
         ...
         ["string.pack"] = function(self, node, a, b, argdelta)
            if #b.tuple < 1 then
               return self.errs:invalid_at(node,
                  "wrong number of arguments (given " .. #b.tuple .. ", expects at least 1)")
            end
            ...
            if packstr.typename == "string" and packstr.literal and a.typename == "function" then
               ...
               if #items ~= #b.tuple then
                  return self.errs:invalid_at(node,
                     "wrong number of arguments (given " .. #b.tuple .. ", expects " .. #items .. ")")
               end
         ...
         ["string.find"] = function(self, node, a, b, argdelta)
            if #b.tuple < 2 or #b.tuple > 4 then
               return self.errs:invalid_at(node,
                  "wrong number of arguments (given " .. #b.tuple .. ", expects at least 2 and at most 4)")
            end
      ...
      function TypeChecker:missing_initializer(node, i, name)
         if self.feat_lax then
            ...
            if node.exps then
               return self.errs:invalid_at(node.vars[i],
         ...
         if decltype then
            ...
            if infertype then
               if infertype.typename == "unresolvable_typearg" then
                  ok = false
                  infertype = self.errs:invalid_at(node.vars[i],
                     "cannot infer declaration type; an explicit type annotation is necessary")
               else
               ...
               if not valnode or valnode.kind ~= "literal_table" then
                  ...
                  if not valnode.is_total then
                     ...
                     if ri.typename == "map" then
                        self.errs:add(var,
                           "map variable declared <total> does not declare values for all possible keys" .. missing)
                        ok = false
                     elseif ri.typename == "record" then
                        self.errs:add(var,
                           "record variable declared <total> does not declare values for all fields" .. missing)
                        ok = false
                     end
                  end
               end
      ...
      visit_node.cbs = {
         ...
         ["forin"] = {
            ...
            before_statements = function(self, node, children)
               ...
               local args = a_type(node.exps, "tuple", {
                  tuple = {
               ...
               if exp1type.typename == "poly" then
                  local _r, f
                  _r, f = self:type_check_function_call(exp1, exp1type, args, 0, nil, nil, exp1,
                     { node.exps[2], node.exps[3] })
                  if f then
                     exp1type = f
                  else
         ...
         ["return"] = {
            ...
            after = function(self, node, children)
               ...
               if n_got > n_expected and (not self.feat_lax) and not vatype then
                  self.errs:add(node,
                     what .. ": excess return values, expected " .. n_expected .. " %s, got " .. n_got .. " %s", expected,
                     got)
               end
               ...
               if n_expected > 1 and
                  ...
                  self.errs:add_warning("hint", node.exps[1].e2,
                     ...
                     node.exps[1].op.op .. "' expression; suggest parentheses if intentional")
               end
            ...
            after = function(self, node, children)
               ...
               for i, child in ipairs(children) do
                  ...
                  elseif decltype.elements and is_numeric_type(cktype) then
                     ...
                     if cv.typename == "tuple" and i == #children and node[i].key_parsed == "implicit" then
                        for ti, tt in ipairs(cv.tuple) do
                           self:assert_is_a(node[i], tt, decltype.elements, node,
                              "expected an array: at index " .. tostring(i + ti - 1))
                        end
                     else
                        self:assert_is_a(node[i], cvtype, decltype.elements, node,
                           "expected an array: at index " .. tostring(n))
                     end
                  elseif node[i].key_parsed == "implicit" then
                     if decltype.typename == "map" then
                        self:assert_is_a(node[i].key, a_type(node[i].key, "integer", {}), decltype.keys, node,
                           "in map key")
                  ...
                  elseif decltype.typename == "map" then
                     ...
                     self.errs:add_in_context(node[i], node, "unexpected key of type %s in table of type %s", cktype,
                        decltype)
                  end
               end
               ...
               if rfieldtype then
                  ...
                  if not ok then
                     if rfieldtype.typename == "poly" then
                        self.errs:add_prefixing(node, err,
                           "type signature does not match declaration: field has multiple function definitions (such polymorphic declarations are intended for Lua module interoperability): ")
                        return
                     end

                     local shortname = selftype and show_type(selftype) or owner_name
                     local msg = "type signature of '" ..
                     node.name.tk .. "' does not match its declaration in " .. shortname .. ": "
                  ...
                  if self.feat_lax or rtype == open_t then
                     ...
                     self.errs:add(node,
                        ...
                        node.name.tk .. "' outside of the scope where '" .. owner_name .. "' was originally declared")
                     return
                  end
               end
            ...
            before_e2 = function(self, node, children)
               ...
               elseif node.op.op == "or" then
                  ...
                  if node.e1.kind == "op" and node.e1.op.op == "and" and
                     ...
                     self:apply_facts(node,
                        facts_not(node, IsFact({ var = node.e1.e1.e1.tk, typ = node.e1.e1.e2.casttype, w = node })))
                  end
               ...
               if node.op.arity == 1 and unop_types[node.op.op] then
                  ...
                  if ra.typename == "map" then
                     if ra.keys.typename == "number" or ra.keys.typename == "integer" then
                        self.errs:add_warning("hint", node,
                  ...
                  if not t then
                     return self.errs:invalid_at(node,
                        "cannot use operator '" .. node.op.op:gsub("%%", "%%%%") .. "' on type %s", ua)
                  end
               ...
               if node.op.arity == 2 and binop_types[node.op.op] then
                  ...
                  if node.op.op == "//" and self.gen_target == "5.1" then
                     if meta_on_operator then
                        ...
                        local div = node_at(node,
                           { kind = "op", op = an_operator(node, 2, "/"), e1 = node.e1, e2 = node.e2 })
                        convert_node_to_compat_call(node, "math", "floor", div)
                     end
                  elseif bit_operators[node.op.op] and self.gen_target == "5.1" then
                     if meta_on_operator then
                        self.all_needs_compat["mt"] = true
                        convert_node_to_compat_mt_call(node, binop_to_metamethod[node.op.op], meta_on_operator, node.e1,
                           node.e2)
                     else
                        self.all_needs_compat["bit32"] = true
                  ...
                  if not t then
                     if node.op.op == "or" then
                        ...
                        if u.typename == "union" and is_valid_union(u) then
                           self.errs:add_warning("hint", node,
                              "if a union type was intended, consider declaring it explicitly")
                        end
                     end
                     return self.errs:invalid_at(node,
                        "cannot use operator '" .. node.op.op:gsub("%%", "%%%%") .. "' for types %s and %s", ua, ub)
                  end
            ...
            for fname, ftype in fields_of(resolved, list) do
               if fields[fname] then
                  if not self:is_a(fields[fname], ftype) then
                     ...
                     self.errs:add(fields[fname], what .. " '" .. fname .. "' does not match definition in interface %s",
                        named)
                  end
               else
